...
const COPY: Readonly<Record<XinmaiJourneySemanticNodeId, Copy>> = Object.freeze({
  ...
  ACTION: c("行动冲动", "分辨最先想推进、退开还是暂停；冲动不等于已经行动。", "保存这次行动冲动观察", "重新选择", "保存后进入记忆联想；真正的小行动会在六项观察完成后再选择。", "方向向量显示靠近、退开或暂停"),
  MEMORY: c("记忆联想", "只区分这更像过去还是现在，不要求披露任何经历。", "保存记忆观察", "暂时分不清", "保存后进入需要与方向；具体记忆不会被记录。", "远近景深帮助区分过去与现在"),
  GOAL: c("需要与方向", "看看这份反应试图保护的需要或价值，不把它变成人格结论。", "保存这次需要与方向观察", "暂时说不清", "第六项保存后只完成观察，仍不会自动形成小行动或现实记录。", "核心光点提示被珍视的方向"),
  ...
  NOT_ATTEMPTED: c("还没有在现实里试过，也没关系。", "这次不会留下成长记录；小行动会被保留，真正试过以后再从返回入口继续。", "先回到生活", "返回重新选择", "两个出口都不会生成新的现实记录或痕迹。", null),
  USER_REJECTED_RECORD: c("不想留下这次记录，也可以。", "确认后本次不记录，已经形成的其他资产不会改变。", "确认不记录，返回旅程入口", "返回重新选择", "两个出口都不会生成新的现实记录或痕迹，也没有惩罚。", null),
  FORMATION: c("正在保存这次现实结果", "只有你刚才确认的事实会进入记录；视觉变化不代表形成已经完成。", "重新保存", "保留已确认内容并离开", "是否可重试严格由真实状态决定。", "痕迹逐渐清晰，但不承担成功判断"),
  CRYSTAL_OWNERSHIP: c("这次现实留下了一道可回看的痕迹", "它记录这次理解如何被现实修正，不是奖励、人格勋章或占卜结论。", "开始下一段现实", "可选：给同行者一个称呼", "上一段已经保留；下一段会从新的现实情境开始。称呼完全可选。", "晶体只是这次模型更新的视觉载体"),
