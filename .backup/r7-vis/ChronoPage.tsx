...
function InitialCoordinatesEntry() {
  ...
  const chronoValves: Array<{ id: ChronoAxis; label: string; value: string; hint: string; range: string; min: number; max: number }> = [
    ...
    { id: "hour", label: "时", value: chronoValues.hour, hint: "［轴向滑动 · 锁定时辰］", range: "十二时辰", min: 0, max: 11 },
  ];
  const activeValveMeta = chronoValves.find((valve) => valve.id === activeAxis) ?? chronoValves[0];
  const activeAxisValue = activeAxis === "hour" ? birthTimeRanges.findIndex((timeRange) => timeRange.label === chronoValues.hour) : chronoValues[activeAxis];
  ...
  function buildCurrentInitialCoordinates(): InitialCoordinates {
    return {
      ...initialCoordinates,
      birthChrono: `${chronoValues.year} / ${padDateUnit(chronoValues.month)} / ${padDateUnit(chronoValues.day)} · ${chronoValues.hour}`,
    };
  }
  ...
  function handleAxisInput(value: number) {
    setChronoValues((current) => {
      ...
      return { ...current, hour: birthTimeRanges[value]?.label ?? current.hour };
    });
  }
  ...
  return (
    ...
    >
      ...
      {!isMotherStage ? (
        <>
          <h1 className="gy-chrono-rail-title">时 序 装 填</h1>
          <section className="gy-chrono-valves" aria-label="初始坐标阀门">
            {chronoValves.map((valve) => {
              ...
              return (
                <button
                  key={valve.id}
                  type="button"
                  className="gy-chrono-valve"
                  data-active={isActive ? "true" : undefined}
                  data-locked={isLocked ? "true" : undefined}
                  onClick={() => handleValveSelect(valve.id)}
                  aria-label={`锁定${valve.label}${valve.value}`}
              >
                  <span>{valve.value}</span>
                </button>
              );
            })}
          </section>
          ...
          >
            <span className="gy-chrono-axis-lock__range">{activeValveMeta.range}</span>
            ...
            />
            <span className="gy-chrono-axis-lock__hint">{activeValveMeta.hint}</span>
          </div>
          <span
          ...
          <button
            className={`gy-causal-rail${isAllValvesLocked ? " gy-causal-rail--active" : " gy-causal-rail--idle"}`}
            type="button"
            onClick={handleEmbedMotherCode}
            disabled={!isAllValvesLocked}
            style={{
              marginTop: 4,
            }}
          >
            <span className="gy-causal-rail__label">确认填装，显影母码</span>
            <span className="gy-causal-rail__dot" />
            <span className="gy-causal-rail__track" />
            <span className="gy-causal-rail__hint">
              {isAllValvesLocked ? "初始坐标已锁定" : "依次锁定四段坐标"}
            </span>
          </button>
        </>
      ) : (
