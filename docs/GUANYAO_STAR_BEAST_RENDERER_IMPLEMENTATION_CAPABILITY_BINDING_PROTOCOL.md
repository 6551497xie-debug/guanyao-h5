...
P48 Binding Resolver → no direct external caller
P48 Binding Result → only P49 Authorization Readiness
```

P48 禁止直接调用 P41–P47，不得绕过冻结链重新计算 Candidate 或 Declaration。

P49 只消费调用方提供的 P48 Result，不直接调用 P48 Resolver；P48 Result 不允许被 Renderer、UI 或 Runtime 绕行消费。
...
6. P48 不调用 P41–P47；
7. P48 Result 只由 P49 Authorization Readiness 消费；
8. P48 不创建实现授权、不选择后端、不执行渲染；
9. P48 gate、P46/P47 gates、freeze gate、release、build 与 `git diff --check` 通过。
