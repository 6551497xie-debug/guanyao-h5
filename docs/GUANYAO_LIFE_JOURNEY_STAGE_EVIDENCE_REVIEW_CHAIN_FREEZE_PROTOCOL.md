...
```text
Authority Declaration Endpoint
...
- Authority Declaration Resolver → 只由 Authority Declaration Endpoint 调用；
- Authority Declaration Result Consumption → 只由 Authority Declaration Endpoint 调用；
- Authority Declaration Endpoint → 当前没有 Runtime、页面或其他业务消费者。
...
Authority Declaration Resolver 位于冻结证据链之外。它只能在 Command 已形成且唯一正式 Authority 再次匹配时构造 P21 Declaration；不得反向修改 Review、Candidate、Readiness 或 Command，也不得生成 Stage Source 或推进 Life Journey Stage。

Authority Declaration Result Consumption 与 Endpoint 同样位于冻结证据链之外。Consumption 只稳定化 Resolver Result；Endpoint 只组合 Resolver 与 Consumption，二者都不得反向修改冻结证据链、生成 Stage Source 或推进 Life Journey Stage。
