...
- 不选择后端、不探测设备、不执行渲染；
- 当前没有 Candidate、Renderer、UI 或 Runtime 消费者。
...
P47 只提供 P46 Backend Capability Reference 的正式语义来源；P46 不得调用 P47 Resolver，Declaration 与 Candidate 的组合必须等待未来独立协议。
...
6. P46 Candidate 没有 Renderer、UI 或 Runtime 消费者；
7. 冻结链不包含 Renderer、UI、Runtime 或 Storage 实现；
8. P47 Resolver 没有下游消费者，且 P46 只导入其 Reference；
9. freeze gate、P39–P47 gates、release、build 与 `git diff --check` 通过。
