...
assert(guide.includes('finalActionState === "PREPARING"') && guide.includes('finalActionState === "SAVING"') && guide.includes("finalDisabled") && guide.includes('aria-busy={') && guide.includes("这一维观察正在准备，尚未保存") && guide.includes("确认保存后才会成为这一维观察"), "pre-persistence state is not visible or accessible");
...
assert(guide.includes('finalActionState === "RETRYABLE"') && guide.includes("重新${grammar.acknowledgementLabel}") && guide.includes('finalActionState === "SAFE_WITHHELD"') && guide.includes("已经保存的观察仍会保留"), "typed recovery/protection is not preserved");
